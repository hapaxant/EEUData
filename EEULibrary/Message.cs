...
namespace EEUniverse.Library
{
    ...
    public class Message : IEnumerable<object>
    {
        ...
        public override string ToString()
        {
            ...
            string DataObjectToString(object value)
            {
                if (value is byte[] bytes)
                {
                    const int amountOfBytesToShow = 4;

                    var len = Math.Min(amountOfBytesToShow, bytes.Length);
                    var bytesspan = new byte[len];
                    for (int i = 0; i < bytesspan.Length; i++)
                    {
                        bytesspan[i] = bytes[i];
                    }
                    return $"[{string.Join(", ", bytesspan)}" +
